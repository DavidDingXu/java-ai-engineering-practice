...
用 IDEA 打开本阶段根 `pom.xml`，每篇文章再进入对应实验。需要外部 Provider 的实验，把地址、模型和凭据填写在实验自己的配置文件后，从 IDEA 运行 `SpringAiAlibabaLabApplication`、`LangChain4jLabApplication` 或 `AgentScopeLabApplication`；MCP 与 A2A 分别运行 `McpLabApplication` 和 `A2aLabApplication`。
