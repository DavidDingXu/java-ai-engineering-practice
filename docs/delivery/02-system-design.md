...
- `ai-gateway-demo`：普通 Chat 调用治理、日志和 trace；结构化输出、Tool、Advisor、Memory 生产接入保留 Spring AI 原生链路。
...
| 模型接入 | 普通 Chat 调用通过治理入口，结构化输出、Tool、Advisor、Memory 和 RAG 保留 Spring AI 原生能力；不允许 Controller 或 Agent 直接依赖厂商 SDK |
