...
Knowledge Service 已实现企业 RAG 主线；Customer BFF 已实现客户身份、委托令牌、完整回答、SSE、短时会话、反馈、重试和升级工单；Ticket Agent Service 已实现可信任务身份、受限步数规划、服务端 Tool 目录、人工确认、幂等、JDBC/Flyway 持久化、审计和 HTTP 下游适配器；JDK8 客户端已能查询任务并提交确认。项目同时提供 AI 安全回归、Micrometer 指标、Agent 并发上限、单一运行配置、框架与协议 labs 和跨平台 release gate。正式 Customer Web、目标 PostgreSQL 容量、真实 IdP/下游联调、持久 UNKNOWN 对账和端到端容量验证仍需在目标公司环境完成。
...
| JDK8 client | 使用真实 JDK8 编译和测试 | 老系统通过版本化 HTTP/OpenAPI 接口接入 |
...
- Provider 协议回归属于测试代码；真实模型 Smoke 与回答 Golden Set 使用受控 API，两者回答的问题不同，不能相互替代。
- PostgreSQL/pgvector 由 `external-integration` Maven profile 和独立 CI 验证；小数据量集成测试不能给出生产容量结论。
