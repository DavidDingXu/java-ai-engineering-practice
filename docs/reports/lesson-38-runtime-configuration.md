...
- 真实模型、PostgreSQL/pgvector、JWT、Token Exchange 和下游地址通过 `JAVA_AI_*` 参数注入。
...
代码回归不访问真实模型和外部基础设施。数据库、IdP、对象存储、业务 Tool、Windows 运行和容量结论仍需要目标环境证据；单一配置只消除装配分叉，不会把测试结果扩大成生产验收。
