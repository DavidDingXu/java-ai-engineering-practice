...
- 合同夹具 5/5 通过，并明确标记 `CONTRACT_FIXTURE`。
- 真实模型 5/5 通过，覆盖正常回答、证据不足拒答、Prompt Injection、敏感凭证和未授权写操作。
- 真实 HTTP 结果包含有效 Trace ID，报告记录模型、Commit、Token、延迟和坏案例。
- Observation 只使用固定操作标签 `context_load` 和 `model_call`，不把问题、Prompt、用户或租户写进指标标签。

## Replacement Boundary

5 条样例只能证明链路和评测方法可用，不能代表生产准确率。公司落地时应从真实坏案例扩充 Golden Set，按业务风险分层，并把模型、Prompt、检索和权限变更纳入同一回归。
