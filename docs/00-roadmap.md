...
| 02 | 模型调用工程化 | 封装模型网关、路由、重试、降级、成本日志 |
| 03 | 流式交互工程化 | 实现 SSE、心跳、断连续传、TTFT 指标 |
| 04 | Prompt 与结构化输出 | 模板化 Prompt，稳定解析模型输出 |
...
| 03 | 不要把大模型调用写进 Controller | ai-gateway-demo | 增加备用模型 Provider |
...
| 12 | AI 输出 JSON 老是坏，Java 后端怎么兜底 | ai-output-demo | 实现 JSON 修复重试 |
| 13 | 结构化输出实战：生成工单处理建议 | ai-output-demo | 输出 TicketAdviceResponse |
| 14 | 结构化输出的单元测试怎么写 | ai-output-demo | 增加坏 JSON 测试用例 |
...
| 60 | Spring AI Advisor 和 Memory 在工程链路里的位置 | ai-gateway-demo | 实现 Gateway 级 Advisor Chain 和 Memory Advisor |
...
| 69 | 写在最后：Java + AI 工程化真正要带走什么 | 全项目 | 按模型网关、Prompt、RAG、Tool、Eval、Trace、发布证据复盘迁移路径 |

## 扩展路线

当前主题覆盖了 Java + AI 工程化落地的主干。后续可以继续沿两个方向扩展：一类是补更多真实业务场景，一类是按团队内部平台能力拆出专门模块，例如模型网关管理台、评测样本管理、知识库运营台和 Trace 查询台。
