...
public final class PgVectorKnowledgeChunkSearchRepository implements KnowledgeChunkSearchRepository {
    ...
    public PgVectorKnowledgeChunkSearchRepository(DataSource dataSource, ObjectMapper objectMapper) {
    ...
    public List<RetrievedKnowledgeChunk> search(
        ...
        PgVectorSearchQuery query = PgVectorSearchQuery.create(
                embedding, embeddingModel, scope, effectiveAt, topK
        );
        try (var connection = dataSource.getConnection();
            ...
            throw new KnowledgeSearchPersistenceException("pgvector knowledge search failed", error);
        }
    }
    ...
    private static void bind(PreparedStatement statement, List<Object> parameters) throws SQLException {
        for (int index = 0; index < parameters.size(); index += 1) {
            ...
            if (value instanceof String text) {
                ...
                throw new IllegalArgumentException("unsupported search parameter type " + value.getClass().getName());
            }
        }
    }
